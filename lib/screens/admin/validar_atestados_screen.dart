...
class ValidarAtestadosScreen extends StatefulWidget {
  ...
  @override
  State<ValidarAtestadosScreen> createState() =>
      _ValidarAtestadosScreenState();
}
  ...
  Future<void> _carregar() async {
    ...
    try {
      final snap = await FirebaseFirestore.instance
          ...
          .get();
      setState(() {
        _faltasComAtestado = snap.docs
            .map((d) => Aula.fromMap(d.data(), d.id))
            .toList();
      });
    } catch (_) {}
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content:
                Text('Atestado aprovado! Reposição criada para a aluna.'),
